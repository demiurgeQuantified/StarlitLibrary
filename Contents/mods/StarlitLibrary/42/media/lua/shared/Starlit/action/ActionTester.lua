...
---@field items table<any, ActionTester.ItemResult[]>
---
---The results of each object requirement, by name of the object requirement.
---@field objects table<any, ActionTester.ObjectResult>
---
...
---@alias ActionTester.ItemResult {item: InventoryItem | nil, success: boolean, validType: boolean, predicates: {[any]: boolean}}
---@alias ActionTester.ObjectResult {object: IsoObject | nil, success: boolean, predicates: {[any]: boolean}}
---@alias ActionTester.ForceParams {objects: {[any]: IsoObject} | nil, items: {[any]: InventoryItem[]} | nil}
...
local function getAllItemsInContainerRecurse(container)
    ...
    while #containers > 0 do
        local container = containers[#containers] ---@as ItemContainer
...
---@return table<string, InventoryItem[] | nil>
local function buildItemTagMap(typeMap)
    ---@type table<string, InventoryItem[] | nil>
    local tagMap = {}

    for _, items in pairs(typeMap) do
        local tags = items[1]--[[@cast -nil]]:getTags()
...
---@field itemsByType table<string, InventoryItem[] | nil> Map of every item in the character's inventory by its full type.
---@field itemsByTag table<string, InventoryItem[] | nil> Map of every item in the character's inventory that has each tag.
local ActionTester = {}
...
---@param requiredObjects table<any, Action.RequiredObject>
---@param result ActionTester.Result
---@param objects IsoObject[]
---@param forcedObjects { [any]: IsoObject } | nil
function ActionTester:_testObjectRequirements(requiredObjects, result, objects, forcedObjects)
    ...
    for name, requirement in pairs(requiredObjects) do
        ...
        ---@type ActionTester.ObjectResult
        local testResult = {
            object = objects[1],
            success = false,
            predicates = {}
        ...
        for i = 1, #objects do
            local object = objects[i] ---@as IsoObject
            local matches = true
...
---@param requiredItems table<any, Action.RequiredItem>
---@param result ActionTester.Result
---@param forcedItems { [any]: InventoryItem[] } | nil
function ActionTester:_testItemRequirements(requiredItems, result, forcedItems)
    ...
    for name, requirement in pairs(requiredItems) do
        ...
        elseif requirement.types then
            items = table.newarray() ---@as InventoryItem[]
        ...
        for i = 1, #items do
            local item = items[i]
            local itemResult = itemResults[matchesFound + 1] ---@as -nil
            ...
            repeat
                ...
                if itemResult.success then
                    if byUses then
                        ...
                        for _ = 2, uses do
                            ---@diagnostic disable-next-line: param-type-not-match, redundant-parameter
...
---@param forceParams ActionTester.ForceParams | nil Items and objects that must be used in the action. If they cannot be used, the function will return early. failedRequirements will only detail why these items were inappropriate.
---@return ActionTester.Result result The result of the test.
---@nodiscard
function ActionTester:test(action, objects, forceParams)
    if not Action.isComplete(action) then
        if DEBUG then
            ...
            log:error("Attempting to test for action %s, but it is incomplete.", action.name)
        end
    end

    ---@type ActionTester.Result
    local result = {
        action = action,
...
---@return ActionTester tester
---@nodiscard
function ActionTester.new(character)
    local items = getAllItemsInContainerRecurse(character:getInventory())
    local itemsByType = buildItemTypeMap(items)
    ...
    return setmetatable(
        {
            ...
            itemsByTag = buildItemTagMap(itemsByType)
        },
        ActionTester
    )
end
