...
---@param objects table<any, ActionTester.ObjectResult>
---@return string
---@nodiscard
...
---@param items table<any, ActionTester.ItemResult[]>
---@return string
---@nodiscard
local function buildItemsString(requiredItems, items)
    ...
    for name, requirement in pairs(requiredItems) do
        ...
        if not showResult then
            showResult = itemResult[1] ---@as ActionTester.ItemResult
        end
            ...
            if requirement.types or requirement.tags then
                ...
                if requirement.types then
                    ...
                    for j = 1, #requirement.tags do
                        tagNames[j] = getText("IGUI_StarlitLibrary_TagDescription_" .. requirement.tags--[[@cast -nil]][j])
                    end
...
---@param testResult ActionTester.Result
---@return string
---@nodiscard
...
---@param testResult ActionTester.Result Result of the action test.
...
---@class Action.ShowFailConditions
---
...
---@field showFailConditions Action.ShowFailConditions | nil
---
...
---@field getTooltipText (fun(state:ActionState):text:string) | nil


---Concrete configuration for when and how a tooltip should be displayed.
---@class Action.TooltipConfiguration : Action.TooltipConfigurationArgs
---@overload fun(config:Action.TooltipConfigurationArgs):self
---
---How to behave when more than one action of this type is available.
---'separate' creates a separate option for each action.
---'submenu' merges the options into a submenu.
...
---
---Conditions for when options for invalid actions should be shown.
---@field showFailConditions Action.ShowFailConditions


---@type Action.TooltipConfiguration
...
---@alias ActionUI.ObjectAction {action: Action, config: Action.TooltipConfiguration}
---@type ActionUI.ObjectAction[]
local objectActions = {}

-- TODO: this doesn't support shit!!! merge it with the object one

---@alias Action.ItemTooltipConditions {itemAs: string}


---@type {action: Action, conditions: Action.ItemTooltipConditions}[]
local itemActions = {}


---@param action Action
---@param tooltipConditions Action.TooltipConfiguration | nil
...
---@param action Action
---@param tooltipConditions Action.ItemTooltipConditions | nil
...
---@param highlight Action.HighlightParams
---@param testResult ActionTester.Result
...
---@param context ISContextMenu
---@param highlighted boolean
---@param action ActionUI.ObjectAction
---@param fail ActionTester.Result
...
---@param context ISContextMenu
---@param testResult ActionTester.Result
---@param config Action.TooltipConfiguration
...
---@return {[ActionUI.ObjectAction]: {successes: ActionTester.Result[], fails: ActionTester.Result[]}}
